...
fun Color.toHex(): String = "0x%04X".format(this.toArgb())
fun Int.toColor(): Color = Color(this)

fun hexToInt(str: String) = java.lang.Long.decode(str).toInt()
fun hexToColor(str: String) = Color(java.lang.Long.decode(str))

fun Int.toHex(): String = "0x%04X".format(this)

fun Int.toHex2(): String = "%02X".format(this)

fun ByteArray.bitsToHex(): String {
    val bitSet = BitSet.valueOf(this)
    Log.d("VRUN", bitSet.toString())
    Log.d("VRUN", bitSet[bitSet.size()].toString())
...
fun ByteArray.decodeSkipUnreadable(): String {
    ...
    this.forEach {
        Log.d("VRUN", this.indexOf(it).toString() + ": " + it.toInt().toString())
    }
